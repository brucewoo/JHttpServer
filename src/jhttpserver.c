...
int get_index_file(char *filename_buf, struct stat *pstat) {
	...
	if (S_ISDIR(stat_buf.st_mode)) {
		// 是目录，追加 index.htm(l)
		strncpy(filename_buf + strlen(filename_buf), INDEX_FILE,
				sizeof(INDEX_FILE));
		// 再次判断是否是文件
		s = lstat(filename_buf, &stat_buf);
...
void read_request(struct process* process) {
	...
	while (1) {
		count = read(sock, buf + process->read_pos,
				kBufferSize - process->read_pos);
		if (count == -1) {
			if (errno != EAGAIN) {
	...
	if (header_length > kBufferSize - 1) {
	...
	if (read_complete) {
		...
		if (c != 0) {
			...
			strncpy(tempstring, c + sizeof(HEADER_IF_MODIFIED_SINCE) - 1,
					time_len);
...
void send_response(struct process *process) {
	...
	while (1) {
		...
		int s = sendfile(process->sock, process->fd, &offset,
...
void handle_error(struct process* process, const char* error_string) {
	cleanup(process);
	perror(error_string);
}
